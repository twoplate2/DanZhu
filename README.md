...
落点由纯物理被动决定(无预定), RTP 精确 80%/120%/200%/360% 四档, 物理表演与操作手感完全真实。
...
游戏内跑分: 长按标题 3 秒。口径 = 纯 Python 物理单线程(无 JIT / 无 C 扩展 /
无 GPU), 每轮 0.7 秒 × 5 轮取中位; 两平台跑的是**同一个 CPython 3.11 + 同一套
物理代码**, 公平对拼。

| 设备 | 芯片 | 每秒模拟步数 | 相对 |
|------|------|-------------|------|
| 台式机 i7-14700KF(20 核, 5.6GHz) | Raptor Lake | 11516(五轮 11132~11807) | 100% |
| 拯救者 Y700 五代 | 骁龙 8 至尊版 Gen5 | ~10000(真机实测) | 87% |

- 每发球约 254 步模拟: 台式机 22ms / 平板 ~25ms 算完。游戏每秒只需 60 步
  (物理固定 60Hz, 与屏幕刷新率无关), **两端余量均约 167 倍**(桌面 192 倍), 实际游玩零差别——跑分比的是余量。
- 平板为何能追平桌面: 纯 Python 负载吃单核 IPC 与缓存(Oryon 大核高 IPC +
  单核 12MB 二级缓存), 桌面的多核与 SIMD 无从发挥; 剩余一成差距来自安卓的
  Python 编译/内存延迟/系统开销。Y700 五代为 2026-07 安兔兔安卓平板性能榜
  第三(401 万, 与榜首同芯片, 仅差调校)。
- ⚠️ **算步/秒别踩坑**: `benchmark_trajectories(duration, runs)` 返回的
  `frames` 是 **runs 轮的总和**, 步/秒应取 `fps_list` 中位数(或
...
## 语音播报 (edge-tts 预录, voice/*.wav 61 个)
...
生成: `python ../tools/generate_voice.py`。61 个语音文件。
