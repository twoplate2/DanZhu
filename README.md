...
("本轮游戏 X 次已结束, 剩余 X 个珠子, 数据已重置"), 播完自动重置。
轮次历史持久化保存(最近 100 条, 重启不丢失)。

## 界面 (5 行全宽上下结构, 上设定/下信息)

```
[顶栏]   音效开关(左) + 每轮X次(左) + 标题居中 + 状态(右)
...
排版: 返还/投入/信息/底行四行首字符统一左沿 24dp(左对齐), 行间距 5dp, 
顶栏标题右偏 36dp 防移动端与左栏按钮重叠, 双因子缩放(_font_scale + _ui_scale)自适应横屏窄屏。

设备能力: 切后台自动静音(SoundPool autoPause)、x10+ 大奖滚分/停留 1.2s、
x2 起中奖震动(45/75/110/150ms)、顶栏声音三态开关(语音已开=绿底,默认 /
音效已开=蓝底 / 音效已关=深底, 点按循环)。

语音播报(edge-tts 预录, voice/*.wav 42 个):
- 中奖: 播"珠子加xx"(15 种 payout 全覆盖, 替换 win 琶音)
- 余额不足: 播"珠子数量不足请重置或降低投入"(替换 error 嗡声)
- 轮次结束: 播"本轮游戏X次已结束,剩余"+数字朗读片段(队列拼接)+"个珠子,数据已重置"
- 轮次设定: 播"每轮已设定为X次"
- 数字朗读: 0~9 / 十百千万 / 两(22 个独立片段, 对标 Clac 项目方案, 不拼接 PCM)

生成: `python ../tools/generate_voice.py`。44 个语音文件(22 原有 + 22 新增)。
...
voice/*.wav                  # 预录语音 42 个(edge-tts, ../tools/generate_voice.py 生成)
...
轮次结束的数字朗读采用队列拼接方案(对标 Clac 项目): 中文文本 tokenize 成 clip key
序列, 按 duration+5ms 间隔依次 `Clock.schedule_once` 排队播放, 不拼接 PCM,
每个片段保持 Xiaoxiao 原声音质。
