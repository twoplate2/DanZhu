...
3. CPU 高压测试的频率采样窗口保持 `[1, 359]` 秒，并改为按绝对墙钟固定 1Hz 采集。

频率采样原先是“读完多核 sysfs，再等待 0.5 秒”，所以真机曾出现 424 点。
现在改为在测试的第 1、2、3……359 秒各读一次，正常情况下固定为 359 个频率点；
若系统某次读取失败或耗时超过 1 秒，则如实少一点，不复制旧值凑数。
