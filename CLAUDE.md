...
**中下部**、弹珠从板面上方雨点般落下堆满杯子 → 装满静止 0.35s(回味) → 0.25s 淡出 →
...
- 时序: ×2 约 2.4s, ×100 约 4.0s(含 0.5s 起播延迟与 0.60s 尾巴)。
...
- **进场/退场是分层动画，不是一根透明度**（`WinPileFX._layers()` 返回 `(压暗alpha, 道具alpha, 缩放k, 位移dy)`）：
  ...
  - 退场：装满后静止 `RESULT_HOLD=0.35` → **透明度走线性**（每帧均匀掉 ~1/15）+ **位移走二次缓出**（上浮 52px、缩到 0.90），`RESULT_FADE=0.25`；压暗层早 0.04s 撤（"灯先亮回来、道具后撤走"）。
  ...
  - ⚠️ **`RESULT_HOLD + RESULT_FADE` 的和（现在 0.60）是"装满到解锁"的全程**，`expected_sec`/`_reveal_deadline`/`fx_probe` 全从这个和推导 —— **不要在别处写死这个数**（`_reveal_deadline` 曾经硬编码 0.45，改尾巴时就会静默脱钩）。`RESULT_HOLD` 是"装满后的回味时间"，调它就是在调回味。
...
- **揭晓合流：中奖的数字/余额/语音一起在"最后一颗球**第一次触地**"那一刻给**（`_reveal_win`；判据见上面的 `_last_touch`）。t=0 只落账务 + 槽闪 + 灯 + 顶栏中间态（`命中 xN · 结算中…`）。以前大字 t=0 就报、语音等装杯播完才念，中间隔一整场（×2≈2.0s、×100≈3.7s），数字等于提前剧透。
  ...
  - 兜底 deadline = `settle + expected_sec(m) - (RESULT_HOLD + RESULT_FADE)`。`expected_sec` 现在**直接读本局真实球数据**（`_last_settle + 0.05`），所以是严格上界。
