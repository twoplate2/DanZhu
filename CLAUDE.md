...
2. **物理层**: `Ball`(__slots__ class, 20属性, 含 `_rng` 撞钉扰动流), `physics_step`, `steer_ball`(碰钉瞬间一次性微调 + 入槽 ALIGN, 飞行途中零干预), `choose_target`, `solve_landing`(修订求解), `benchmark_trajectories`
   ...
   - **预发射修订求解**(`solve_landing`): **零注入优先** —— 换种子(≤20, 盘面不动)找纯物理轨迹
     (99.3% 零注入, 平均预演 ~2 次); 全部不中才注入兜底(最后碰撞点, dvx≤150)。
     兜底全失败(<0.1%)转物理落格结算, 绝不穿帮。真发按 (seed, 注入点k, dvx) 重放:
     `_frame` 累加器循环内计数撞钉, 第 k 次碰撞帧末 `b.vx += dvx`
...
**修订求解成功率=100%**(300/300, 零注入优先: 注入率≤5% + 注入量≤150 + 平均预演≤7次, 盘面不动)。
修订前命中率>35%(宽松哨兵, 防物理退化)。
**下落节奏门禁**(专家组标定口径: power=0.8 + 居中槽 + 固定种子): 碰钉减速比 0.45~0.70
(防黏滞<0.45/防穿阵>0.70) + 滞留帧≤8/发 + 行穿行≥0.15s。
