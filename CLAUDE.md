...
- **彩蛋（弹珠返回发射槽）走 `_easter_hold` 全套锁**：它不设 `_land_hold`，不锁的话 `park_ball` 会在弹窗还开着时（`landed_at+0.6s`）就跑掉——重掷盘面、state 回 `ready`、按钮恢复，然后装杯才播。锁从 `settle()` 持续到装杯播完（`_easter_finish`）。这条路径**不设状态栏文字**（结果全交给弹窗说），也**不播装杯之外的额外语音**——语音就是弹窗那句话（`voice_easter_{1,10,50,100}`，按投注档穷举）。弹窗关掉后才补播 ×2 的装杯（`_on_easter_closed`），装杯落定才响 ×2 轻赢音。
