...
- 时序: ×2 约 2.2s, ×100 约 3.8s(含 0.5s 起播延迟与 0.45s 尾巴; 整场雨前移后又短了 0.2~0.55s)。
...
- **揭晓合流：中奖的数字/余额/语音一起在"最后一颗球落定"那一刻给**（`_reveal_win`）。t=0 只落账务 + 槽闪 + 灯 + 顶栏中间态（`命中 xN · 结算中…`）。以前大字 t=0 就报、语音等装杯播完才念，中间隔一整场（×2≈2.0s、×100≈3.7s），数字等于提前剧透。
  ...
  - 兜底 deadline = `settle + expected_sec(m) - 0.45`。`expected_sec` 现在**直接读本局真实球数据**（`max(b["end"]) + 0.05`），所以是严格上界。
