...
1. **常量+几何**: 520×660 逻辑坐标系(y 向下)。发射槽在井底(PLUNGER_Y=FLOOR-BALL_R-2, 弹簧 Z 字形露出), 发射速度 1180~1220
...
**弧面接触率≈100%**、冲顶x跨度≥20px、首钉55~75帧、转向每发都有、
首钉前无碰撞段折角≤4°/相邻帧差≤1°、音效体检0异常。
**修订求解成功率=100%**(300/300, 定向修正+换种子, 平均预演≤7次, 盘面不动)。
修订前命中率>35%(宽松哨兵, 防物理退化)。GUI 真发验证: 落格==结算槽 0 穿帮。
