...
- 时序: ×2 约 2.0s, ×100 约 3.6s(含 0.5s 起播延迟与 0.45s 尾巴)。
- **T(整场演出的收尾时刻)= 最后一颗球"第一次触地"**(`_last_touch`), 不是"最后一颗弹完"(`max(end)`)。
  实测把判据换成 `_last_touch` 后整场短 0.22~0.31s(x2 1.71→1.46s, x100 3.30→3.08s), 而弹跳只是装饰。
  ⚠️ **弹跳尾巴要能在解锁前播完**, 所以 `tick()` 的 result 段也在调 `_advance_balls()` —— 不推进的话
  `_ball_screen` 对未落定球是按 tt 插值的, 它们会**冻在半空**而不是落到堆里(`fx_probe` [5] 有门禁:
  最长尾巴 0.31s < RESULT_HOLD+RESULT_FADE=0.45s)。
...
- **进场/退场是分层动画，不是一根透明度**（`WinPileFX._layers()` 返回 `(压暗alpha, 道具alpha, 缩放k, 位移dy)`）：
  ...
  - 退场：静止 `RESULT_HOLD=0.20` → **透明度走线性**（每帧均匀掉 ~1/15）+ **位移走二次缓出**（上浮 52px、缩到 0.90），`RESULT_FADE=0.25`；压暗层早 0.04s 撤（"灯先亮回来、道具后撤走"）。
...
- **揭晓合流：中奖的数字/余额/语音一起在"最后一颗球**第一次触地**"那一刻给**（`_reveal_win`；判据见上面的 `_last_touch`）。t=0 只落账务 + 槽闪 + 灯 + 顶栏中间态（`命中 xN · 结算中…`）。以前大字 t=0 就报、语音等装杯播完才念，中间隔一整场（×2≈2.0s、×100≈3.7s），数字等于提前剧透。
  ...
  - 兜底 deadline = `settle + expected_sec(m) - 0.45`。`expected_sec` 现在**直接读本局真实球数据**（`_last_touch + 0.05`），所以是严格上界。
