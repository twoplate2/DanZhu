...
class FpsCurve(Widget):
    ...
    # ⚠️⚠️ **玩家定的就是 3, 别再动它** —— 他 2026-09-17 原话:「3像素挺好, 别改了, 我就要 3」。
    #    我上一版把它改成 6, 是**误判**: 我拿 Read 看渲染出来的 PNG, 而那张图 15889px 宽、
    #    显示时被**缩到 ~1000px**, 于是 3px/帧的图在我眼里糊成一片, 我据此断定"要 6px 才
    #    看得清"。玩家是在**原尺寸**下看的, 3px 完全够。
    #    ⇒ 判断"清不清楚"必须**裁一小块按原尺寸看**, 不许看整张被缩放的图(这个坑本会话
    #      踩了两次, 另一次是 `temp/scan_x1.png`)。
