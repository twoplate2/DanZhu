...
class FpsCurve(Widget):
    ...
    #       索引少 34 倍)又不会撞这个上限。小图(_grp 降采样后几百点)与成绩面板
    #       (300 来点)没撞到过, 但它们用的是同一个 `joint="round"`, 点数涨上去同样会中招。
    ...
    def __init__(self, gaps_ms, cap_fps=120.0, tags=None, on_zoom=None,
                 frame_spaced=False, line_w=1.15, **kw):
        super().__init__(**kw)
        ...
        # 曲线线宽。⚠️ **默认值 1.15 就是老观感**(Kivy 里实际画成 2.3 像素粗, 见
        #    `ZOOM_PER_FRAME_PX` 那段)。玩家 2026-09-17 明确说「别动」⇒ 吃这个默认值的
        #    那张图(弹窗小曲线, `_show_fps_curve`)像素一个都不许变。
        #    ⚠️ **成绩面板那条趋势线不归这里管** —— 它是另一个控件 `SpeedCurve`, 线宽是
        #       它自己硬编码的 `width=1.15`。两处数值今天相同, 但**不是同一个旋钮**:
        #       改这里的默认值不会连带它, 改它也不会连带这里。
        #    只有放大图显式传 `line_w=1.0`(真 1 像素 + 点间距 2 ⇒ 留出 1 像素空隙)。
    ...
    def _draw(self, *_):
        ...
        with self.canvas:
            ...
            if self._gaps:
                ...
                if len(_pts) >= 4:
                    # ⚠️ 线宽走 `self._line_w`(默认 1.15 = 弹窗小图的老观感, 见
                    #    `__init__` 里那段; 放大图传 1.0)。写成 1.0 时 Kivy 会走
                    #    GL_LINES 老路 ⇒ 真 1 像素, 此时 `joint` 无意义(那一版没有关节圆盘)。
                    #    ⚠️ 别按直觉把 1.0 "补"成 1.15 —— Kivy 的 width 是**半宽**。
...
class SpeedCurve(Widget):
    ...
    ⚠️ **线宽与 `FpsCurve` 的默认值一致**(`width=1.15, joint="round"`) —— 玩家 2026-09-16:
       「这个曲线是不是**太粗**了, 你看看之前的那个帧率曲线用哪个粗细, 这个也那个粗细」。
       ⚠️ 2026-09-17 之后 **放大图那一档变成了 1.0**(真 1 像素, 见 `ZOOM_PER_FRAME_PX`
       那段: Kivy 的 width 是半宽), 而弹窗小图与**本图**仍是 1.15(= 实际 2.3 像素粗)。
       三个刻度别混: 这里是硬编码的 1.15, 不是 `FpsCurve(line_w=...)`。
...
class RootWidget(BoxLayout):
    ...
    def _fps_curve_zoom(self, curve):
        ...
        sv.add_widget(FpsCurve(gaps, cap_fps=getattr(curve, "_cap", 120.0),
                               ...
                               frame_spaced=True,
                               # ⚠️ **只给放大图**传 `line_w=1.0`(真 1 像素)。小图吃
                               #    `FpsCurve` 的默认值 1.15(= 实际 2.3 像素); 成绩面板那条
                               #    是 `SpeedCurve` 自己硬编码的 1.15 —— 两处都保持原样,
                               #    玩家 2026-09-17: 「别动」。见 `ZOOM_PER_FRAME_PX` 那段。
                               line_w=1.0,
