...
def _hist_stamp(raw):
    """把记录里的时间戳**归一化**成 `09-15 14:07`(本年) / `2025-11-03 20:41`(往年)。

    玩家 2026-09-15 三次定案, 版本演进是: ①「只有月日 2 个数值」→ ②「日期改为年月日
    时间(看起来宽度足够)」→ ③ **实测发现②的"宽度足够"不成立**(那一版整行 364px,
    而 360dp 上内容区只有 275.2px ⇒ 字号被 `_fit_uniform` 从 14sp 压到 **11.48sp**,
    已低于 Android 正文建议下限 12sp), 于是玩家选了**本年省年份**。

    ⚠️ **为什么"当年省年份"是安全的**: 历史只留 100 条, 而这张表是"最近几天"的读数,
       绝大多数行都是本年 —— 省掉 `2026-` 换来的是**字号从 11.48sp 回到 ~12.8sp**。
       而**跨年的行照旧带年份**, 所以"09-15 分不出今年去年"这个歧义**没有被引入**。
