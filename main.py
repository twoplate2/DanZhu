...
# ⚠️ 这一页**一上来六个字就全亮**(没有淡入): 系统那张 presplash 里已经把同一行字画好了
#    (同一个位置、同一个尺寸、同一个颜色 —— 见 `android/presplash.png`), 我们这页如果从
#    alpha=0 再淡进来, 交接处就会"字先消失一下再出现"。所以 `lit` 恒为 1。
...
def _veil_title_state(t):
    ...
    **圆点左边的字是这一轮的新颜色、右边还是旧的**; 走到头再从左边重来一轮,
    ...
    返回 `(lit, fill, grade, rl)`:
      · `lit` = **恒为 1**(这一页不做淡入: 系统那张 presplash 已经把同一行字画在同一位置上了);
    ...
    # ① 一上来就是满亮 —— 见上面 `VEIL_TITLE_IN_SEC` 那段注释(和系统那张 presplash 对齐)。
    lit = 1.0
