...
class FpsCurve(Widget):
    ...
    # 抖动带的透明度(玩家 2026-09-16 选定的画法: 最慢~最快画成带, 中位画成实线)。
    # ⚠️ **必须半透明**: 带子每列一根、铺满整幅宽 —— 用不透明的色就等于把原来那块糊出来的
    #    实心色块**原样画回来**(只是从"意外糊了"变成"有意糊了")。
    BAND_ALPHA = 0.30

    # 每列**至少**攒够这么多帧才聚合 ⇒ 列数 = min(横轴像素数, 总帧数 / 本值)。
    # ⚠️ **这个数不能小**。逐帧瞬时帧率本身噪声极大 —— 165Hz 满帧设备上实测 51% 的帧正好
    #    165、其余散在 150~165。一列只攒 7 帧的话, **连中位数都在 165 和 150 之间反复跳**
    #    ⇒ 中线自己就长成一排锯齿, 还会把带子整个盖住, 等于白改(实测截图确认过)。
    #    攒到十几帧中位才稳得住。
    # ⚠️ 也不能大到把时间分辨率丢光: 列数上限仍是横轴像素数, 所以宽屏不会变粗;
    #    只有**帧数本来就少**的机器(低帧率档)才会列数不足 —— 那时的带子呈阶梯状, 可接受。
    MIN_FRAMES_PER_COL = 16
    ...
    def _draw(self, *_):
        ...
        with self.canvas:
            ...
            if self._gaps:
                ...
                # ⚠️⚠️ **必须按像素列聚合, 不能"一帧一个点"**(2026-09-16 修)。
                #    病根: 165Hz 设备上一轮 **5263 帧 / 横轴 785 像素 = 每列塞 7 帧**,
                #    而这一列里那 7 个点的纵向跨度中位就有 **9fps(≈16 像素)** —— 逐帧折线
                #    在同一列里上下横跳、自己跟自己交叉, **糊成一块实心色块**: 深色从图表顶
                #    一直铺到 150 那格, 看起来像"帧率一直在 150 上下剧烈波动"。
                #    可那一轮实测**平均 164.9 / 中位 165.1**, 基本满帧跑满 —— **图在骗人**;
                #    真正该被看见的长卡顿(那几根伸到 90 以下的尖刺)反倒被糊掉的地基淹了。
                #    (玩家 2026-09-16 的原话就是"我发现一个帧率曲线的绘制问题"。)
                #    ⇒ 每列只留三个数: **最慢 / 最快 / 中位**。
                #      · 最慢~最快 画成一条**半透明抖动带**(同 PresentMon 那类专业帧率工具的
                #        画法): 带子越厚 = 那一刻抖得越厉害;
                #      · **中位**画成实线叠在带子上 —— 它的位置才是"那一刻到底跑得怎么样"。
                #    ⚠️ 中位线是**每列各取各的**, 不是全局中位 —— 全局中位会变成一条平线,
                #       那就把"哪一段时间在抖"整个丢了。
                _nb = max(1, min(int(pw), len(self._gaps) // self.MIN_FRAMES_PER_COL))
                _cols = [None] * (_nb + 1)             # 每列: [最慢, 最快, [原始值...]]
                _el = 0.0
                for _gap in self._gaps:
                    _v = max(lo, min(hi, 1000.0 / _gap))
                    _c = int(_el / max(0.01, total_ms) * _nb)
                    if _c > _nb:
                        _c = _nb
                    _b = _cols[_c]
                    if _b is None:
                        ...
                        _b[2].append(_v)
                    _el += _gap
                # 空列**前向填充**再反向补头 —— 一列一个点的情况下(低帧率段 + 宽屏,
                # 例如 10fps 跑分那 5 轮)空列能占七成, 留缝的话带子会碎成一片虚线。
                # 填充用的是**相邻列的真数据**, 不是插值: 那一列本来就没有帧, 没有"真值"可取。
                _prev = None
                for _c in range(_nb + 1):
                    if _cols[_c] is None:
                        _cols[_c] = _prev
                    else:
                        _prev = _cols[_c]
                _first = next((b for b in _cols if b is not None), None)
                if _first is not None:
                    for _c in range(_nb + 1):
                        if _cols[_c] is None:
                            ...
                            break
                # 抖动带: 每列一个半透明矩形, 下沿=该列最慢帧、上沿=最快帧。
                # ⚠️ **别改成 `Mesh`**(2026-09-16 实测在 Kivy 2.3.1 上**根本不渲染**:
                #    同时画的 `Line` 好好在屏幕上, 同场景的 `Mesh(mode='triangle_strip')`
                #    一个像素都没出 —— 它没绑纹理时 sampler 取不到东西)。
                # ⚠️ 列宽 `+0.6px` 是**盖缝**用的: 相邻矩形共享一条边, 浮点取整会在中间
                #    留出 1px 亮线, 整幅宽排下来就是一片竖条栅栏。
                _cw = pw / _nb
                _mids = []
                Color(0.24, 0.29, 0.35, self.BAND_ALPHA)
                for _c in range(_nb + 1):
                    _b = _cols[_c]
                    if _b is None:
                        continue
                    Rectangle(pos=(x0 + _c * _cw, y0 + _b[0] / hi * ph),
                              size=(_cw + 0.6,
                                    max(0.8, (_b[1] - _b[0]) / hi * ph)))
                    _vs = sorted(_b[2])
                    _mids.extend([x0 + (_c + 0.5) * _cw,
                                  y0 + _vs[len(_vs) // 2] / hi * ph])
                # 中线: 浅底上使用深灰蓝，尖峰足够清楚，又不抢走坐标与统计信息。
                Color(0.24, 0.29, 0.35, 1)
                if len(_mids) >= 4:
                    Line(points=_mids, width=1.15, joint="round")
