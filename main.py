...
class FpsCurve(Widget):
    ...
    # ⚠️⚠️ **玩家定的值是 3** —— 2026-09-17 二次定案: 「把放大后的帧率曲线从 2 个像素
    #    1 组改为 3 个像素一组」。这个旋钮他来回拧过四次(2 -> 6 -> 3 -> 2 -> 3), 别拿
    #    "上一版是几"当依据, **只认这一行的值**。他看过 1/2/3/6 四张对比图
    #    (`output/fps_raw_1to{1,2,3,6}.png`):
    #      · 1px 不行 —— 点间距 1 比线宽 1.15 还小, 相邻线段完全重叠, 曲线糊成一条粗毛线;
    #      · 3px 是他**最终**要的 —— 空隙够用, 原尺寸下每一帧都分得开;
    #      · 2px 停过一版(v0.8.14, 当时理由是"比 3px 少拖 5 屏"), 已按上面那条改回 3px;
    ...
    #    一片, 我据此改成 6px。玩家在**原尺寸**下看的, 3px 完全够。**这个坑本会话踩了两次**。
    # ⚠️ 它同时决定要横向拖多久: 5263 帧 x 3px ≈ 15789px, 手机一屏约 1220px ⇒ 约 13 屏
    #    (2px 那版约 8 屏)。
            ...
            if self._gaps:
                ...
                for _gap in self._gaps:
                    ...
                    if _n >= _grp:
                        ...
                        _pts.extend([x0 + _el / max(0.01, total_ms) * pw,
                                     y0 + _v / hi * ph])
                        _worst = 0.0
                        _n = 0
...
class RootWidget(BoxLayout):
    ...
    def _fps_curve_zoom(self, curve):
        ...
        ⚠️ **代价是实测过的, 别当它是 bug**: 1:1 时相邻两点的 y 差**就是数据本身的逐帧
           ...
           3.0 = 每帧一个点 / 0.5 = 每 2 帧 / 0.34 = 每 3 帧。
        ...
        #       `ZOOM_PER_FRAME_PX`(=3) 像素, 不是"1 像素 1 帧" —— 多出来的那 2 像素是
