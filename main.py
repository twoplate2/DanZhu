...
class Sfx:
    ...
    SFX_READY_POLL = 0.05               # 探测间隔(秒) —— 它直接决定**摘页的发现延迟**。
    #   ⚠️ 0.15 -> 0.05(2026-09-11, v0.6.44): 原来的 0.15 是随手定的、没有依据(注释里就写着
    #      "探测间隔"四个字)。而音频是**异步解码**的, 探针只是在轮询"好了没有" ——
    #      若它在探针刚过去 1ms 时就绪, 玩家要白等满一个间隔(0.15s; 平均 75ms)。
    #      压到 0.05 后最坏 50ms、平均 25ms。加载页的等待本来就是玩家最敏感的那几秒。
    #   ⚠️ **为什么加密不贵**: `probe_all()` 遇到第一个"还不能播"的 sample 就 `return False`
    #      (早退), 所以"还没就绪"期间的每次探测通常只花一两次 play 调用; 只有**最后一次全就绪**
    #      才需要遍历完(41 音效 + 57 语音 = 98 个 sid)。这不是把后台线程压满的活。
    #   ⚠️ 别再往上调回"间隔大一点省点 CPU": 省下的 CPU 换不来任何东西, 而多等的那几十毫秒
    #      是**每一次冷启动**都要付的。
