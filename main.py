...
def build_deflectors():
    ...
    现在的弧面是量出来的: 取 9 档蓄力的上升段包络, 球心沿转弯外法线偏移 BALL_R*BALL_VIEW+描边半宽
    = 16.1px, 再取每 y 最大的 x 得到包络面, 采样 28 个控制点。27 段短线 + 圆头圆角 Line 渲染 →
    肉眼无折角。包络保证 0 碰撞(1000 发零撞弧), 视觉间隙 -1~8px 全程贴合。
    弧面仍是真碰撞体: 画出来的东西必须是实心的, 否则哪天有球被钉子弹上来穿过去就是当场说谎。
    selftest 的天花板撞击率门禁(<10%)替这条守着。要改弧面形状必须重新量包络。"""
