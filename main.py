...
def build_deflectors():
    ...
    老版 (510,108)→(450,88) 横切轨迹, 球必须 6 帧内横移 37px 从左侧挤出去, 逼出 38° 折角,
    自身从未被球碰到。既挡路又解释不了转向。

    当前 28 点/27 段, 由 9 档蓄力上升段包络(每 y 取最大 x)沿外侧偏移
    BALL_R(9)×BALL_VIEW(1.4)+描边半宽(3.5)=16.1px 得到。最弱蓄力弯道处球心距
    ~9px(物理净空约 0), 设计上靠上升段零随机保证不真碰, 实测 1000 发 0 撞弧。
    弧面是碰撞体: 画出来就必须是实的。改形状必须重新量包络。"""
    cpts = [(503, 171), (503, 169), (503, 167), (503, 164), (503, 162),
            (503, 159), (503, 157), (503, 154), (503, 151), (503, 148),
            (503, 143), (502, 138), (502, 132), (499, 128), (495, 118),
            (494, 112), (491, 104), (487, 100), (478, 95), (472, 84),
            (464, 78), (460, 73), (451, 68), (450, 64), (438, 60),
            (434, 56), (432, 53), (426, 51)]
...
class GameArea(FloatLayout):
    ...
    def _redraw(self, *_):
        ...
        with self.canvas:
            ...
            # 发射区导流弧(28点/27段, 贴着球的越顶轨迹外侧; 见 build_deflectors)
            if g.geo["deflectors"]:
                Color(*hex_rgb(COL_WALL))
                pts = []
