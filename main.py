...
# (盘面倍率表见 roll_multipliers 上方的 VALUE_SHAPE / K_DIST / DUD_Q)
...
# 盘面生成 = 两分支。掷 k 个奖格后:
#   以概率 q     -> 整盘 k 格全填 x2 (废盘分支)
#   以概率 1-q   -> 每格独立抽倍率, 抽到"全是 x2"就重抽(即取"非全x2"的条件分布)
# 于是废盘率精确 = q, 而盘面仍能同时出现 x2/x3/x5/x10 —— 多样性和废盘率控制不再二选一。
# (曾经把结构定死成"1个非x2格 + 其余全x2", 废盘率是可控了, 但每盘最多只有两种数字,
#  3种以上占比恒为 0%, 玩家一眼就看出来了。)
VALUE_SHAPE = {   # 非 x2 部分的形状(条件分布, 和=1)。档位越高越舍得给大数字。
    0.80: {3: 0.88, 5: 0.08, 10: 0.03,  20: 0.01},
    1.20: {3: 0.62, 5: 0.24, 10: 0.11,  20: 0.03},
    2.00: {3: 0.44, 5: 0.28, 10: 0.175, 20: 0.06,  50: 0.032, 100: 0.013},
    3.00: {3: 0.37, 5: 0.29, 10: 0.195, 20: 0.085, 50: 0.038, 100: 0.022},
}
K_DIST = {        # 每盘有奖格数分布(铺宽, 让盘面有疏密变化)。均值 E[K] = 中奖率x9。
    0.80: {1: 0.06, 2: 0.30, 3: 0.52, 4: 0.12},   # E[K]=2.70 -> 中奖率 30.0%
    1.20: {2: 0.08, 3: 0.37, 4: 0.42, 5: 0.13},   # E[K]=3.60 -> 40.0%
    2.00: {2: 0.06, 3: 0.20, 4: 0.55, 5: 0.19},   # E[K]=3.87 -> 43.0%
    3.00: {3: 0.06, 4: 0.20, 5: 0.52, 6: 0.22},   # E[K]=4.90 -> 54.4%
}
DUD_Q = {0.80: 0.10, 1.20: 0.08, 2.00: 0.04, 3.00: 0.02}   # 全 x2 废盘率(精确成立)


def _board_ev(p2, rtp):
    """给定 x2 权重 p2, 闭式算出该档的 RTP(= E[盘面倍率和]/9)。
    E[和|k格,非全x2] = (k*E[v] - 2k*p2^k) / (1 - p2^k);  E[盘|k] = q*2k + (1-q)*该条件期望。"""
    shape, kd, q = VALUE_SHAPE[rtp], K_DIST[rtp], DUD_Q[rtp]
    ev = 2 * p2 + (1 - p2) * sum(v * w for v, w in shape.items())
    total = 0.0
    for k, pk in kd.items():
        dud = p2 ** k                                  # k 格全 x2 的自然概率
        cond = (k * ev - 2 * k * dud) / (1 - dud)
        total += pk * (q * 2 * k + (1 - q) * cond)
    return total / NUM_SLOTS


def _solve_p2(rtp):
    """二分反解 x2 权重, 使 _board_ev 精确等于档位。x2 权重不手写就不会手滑写漂,
    改 VALUE_SHAPE/K_DIST/DUD_Q 任何一项都会自动重新配平。"""
    inc = _board_ev(0.9, rtp) > _board_ev(0.1, rtp)    # 先探单调方向
    lo, hi = 0.0, 1.0 - 1e-12
    for _ in range(200):
        mid = (lo + hi) / 2
        if (_board_ev(mid, rtp) < rtp) == inc:
            lo = mid
        else:
            hi = mid
    return (lo + hi) / 2
...
for _rtp in (0.80, 1.20, 2.00, 3.00):
    ...
    assert abs(_board_ev(_p2, _rtp) - _rtp) < 1e-9, "RTP 漂移: %.4f" % _rtp


def roll_multipliers(rtp=0.80):
    """每盘独立抽 k 个格的倍率(天然多档次, 一盘可同时出现 x2/x3/x5/x10);
    以概率 DUD_Q 走"整盘全 x2"废盘分支, 否则抽到全 x2 就重抽 ——
    于是废盘率精确 = DUD_Q[rtp], RTP 精确 = 档位(见上方断言), 不需要保底/冷却修正。"""
    k = _pick(K_DIST.get(rtp, K_DIST[0.80]))
    dist = VALUE_DIST.get(rtp, VALUE_DIST[0.80])
    if random.random() < DUD_Q.get(rtp, 0.0):
        vals = [2] * k
    else:
        vals = [_pick(dist) for _ in range(k)]
        while all(v == 2 for v in vals):      # 拒绝采样 = 取"非全x2"条件分布
            vals = [_pick(dist) for _ in range(k)]
    mult = [0] * NUM_SLOTS
