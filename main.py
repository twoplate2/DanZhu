...
def build_deflectors():
    ...
    当前弧面: 9档蓄力上升段包络(取每y最大x)→外法线偏移16.1px→窗口平滑→52点/51段。
    偏移量 = BALL_R(9)×BALL_VIEW(1.4)+描边半宽(3.5)=16.1px。弯道区(y 64~128)右移5px
    补强净空(审查发现原版仅9.3px)。实测: 球心距≥12.7px(物理净空3.7px), 900发零撞弧。
    顶部(y<66)弧面比球路径宽30~130px, 有意留空——球在此处已越过顶点开始下落,
    弧面收窄会让下降轨迹擦碰。selftest 天花板门禁(<10%)守着弧面安全。
    要改形状必须重新量包络, 手摆控制点会重现老版"挡路又没用"的状态。"""
    cpts = [(503, 171), (503, 170), (503, 169), (503, 168), (503, 167), (503, 165),
            (503, 164), (503, 163), (503, 162), (503, 160), (503, 159), (503, 158),
            (503, 157), (503, 155), (503, 154), (503, 152), (503, 151), (503, 149),
            (503, 148), (503, 146), (503, 143), (502, 140), (502, 137), (502, 135),
            (502, 132), (501, 126), (500, 121), (500, 115), (499, 112), (498, 108),
            (496, 105), (494, 102), (492, 99), (488, 97), (485, 93), (480, 89),
            (478, 85), (474, 81), (471, 79), (468, 76), (464, 74), (459, 71),
            (456, 69), (455, 66), (452, 64), (446, 62), (439, 60), (436, 58),
            (434, 56), (434, 54), (431, 53), (426, 51)]
...
class GameArea(FloatLayout):
    ...
    def _redraw(self, *_):
        ...
        with self.canvas:
            ...
            # 发射区导流弧(52点/51段, 贴着球的越顶轨迹外侧; 见 build_deflectors)
            if g.geo["deflectors"]:
                Color(*hex_rgb(COL_WALL))
                pts = []
